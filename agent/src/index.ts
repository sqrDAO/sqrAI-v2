...
import { TwitterClientInterfaceV2 } from "@sqrdao/twitter-client";
import {
    AgentRuntime,
    CacheManager,
    Character,
...
import { twitterPlugin } from "@sqrdao/twitter-client";
import { BullMQPlugin } from "@sqrdao/plugin-bullmq";
import { calendarPlugin } from "@sqrdao/plugin-calendar";
import { firecrawlPlugin } from "@sqrdao/plugin-firecrawl";
import { reasonPlugin } from "@sqrdao/plugin-reason";
...
export async function initializeClients(
    ...
    if (clientTypes.includes(Clients.TWITTER)) {
        const twitterClient = await TwitterClientInterfaceV2.start(runtime);
...
export async function createAgent(
    ...
    return new AgentRuntime({
        ...
        plugins: [
            ...
            calendarPlugin,
            reasonPlugin,
            getSecret(character, "FIRECRAWL_API_KEY") ? firecrawlPlugin : null,
        ].filter(Boolean),
        providers: [],
        actions: [],
        services: [],
